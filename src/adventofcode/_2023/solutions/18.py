...
class Lagoon(BaseMatrix):
    ...
    def dig_plan(self):
        ...
        for direction, amount, _ in self.lines:
            ...
            original_new_loc = loc[0] + dir_tuple[0] * amount, loc[1] + dir_tuple[1] * amount
        ...
        self.data = self.data[np.min(np.argwhere(self.data == '#')[:, 0]):np.max(np.argwhere(self.data == '#')[:, 0]) + 1, np.min(np.argwhere(self.data == '#')[:, 1]):np.max(np.argwhere(self.data == '#')[:, 1]) + 1]
        print(self)
